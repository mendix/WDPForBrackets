        ...
        readMendixPackageXMLFile : function (xmlDoc) {
            ...
            $('#mx_properties .mx-coder-func-item input').each(function (index, inputNode) {
                $(inputNode).on('blur', function () {
                    //TODO: clean this up?
                    if (this.id.indexOf('Name') > -1) {
                        if (this.value.trim() !== clientModuleName.trim()) {
                            clientModule.setAttribute('name', this.value.trim());
                        }
                    } else if (this.id.indexOf('Version') > -1) {
                        if (this.value.trim() !== clientModuleVersion.trim()) {
                            clientModule.setAttribute('version', this.value.trim());
                        }
                    }
