...
define(function (require, exports, module) {
    ...
    var MODULE_NAME_CODER               = 'mendix.coder',
    ...
    var MendixCodeManager = {
        ...
        setupEvents : function () {
            ...
            $(currentEditor).on('keyEvent', MendixCodeManager.keyEventHandler);
            $(EditorManager).on('activeEditorChange', MendixCodeManager.activeEditorChangeHandler);
                
        },
         
        refreshMendix : function () {
            $('#mx_coder_iframe').attr('src', "http://localhost:8080/");
        ...
        readMendixXMLFile : function () {
            ...
            packageXml = xmlDoc.getElementsByTagName("package");
            
            // Check if we have a widget xml file.
            widgetXml = xmlDoc.getElementsByTagName("widget");
        ...
        fileOpenType : function () {
            ...
            if (file !== '') {
                ...
                console.log('Found the following file: ' + file + ' - with extension - ' + extension);
                switch (extension) {
                    case 'js':
        ...
        activeEditorChangeHandler : function ($event, focusedEditor, lostEditor) {
            ...
            if (lostEditor) {
                $(lostEditor).off("keyPress", MendixCodeManager.keyEventHandler);
            }

            if (focusedEditor) {
                MendixCodeManager.fileOpenType();
                $(focusedEditor).on("keyPress", MendixCodeManager.keyEventHandler);
            }
